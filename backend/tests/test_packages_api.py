...
def test_list_and_get_package_include_documents() -> None:
    created = client.post(
        "/api/v1/packages",
        json={"authority_code": "authority_a", "name": "Listable Package"},
    )
    ...
    assert any(item["id"] == package_id for item in listed.json())

    missing = client.get("/api/v1/packages/00000000-0000-0000-0000-000000000001")
    ...
    detail = client.get(f"/api/v1/packages/{package_id}")
    assert detail.status_code == 200
    body = detail.json()
    assert body["document_count"] == 1
    assert body["documents"][0]["filename"] == "cover_letter.pdf"


def test_findings_include_approval_state() -> None:
    package_id = client.post(
        "/api/v1/packages",
        json={"authority_code": "authority_a", "name": "Approval State Package"},
    ).json()["id"]
    client.post(
        f"/api/v1/packages/{package_id}/documents",
        json={
    ...
    run_id = client.post(f"/api/v1/packages/{package_id}/validate").json()["id"]
    findings = client.get(
        f"/api/v1/packages/{package_id}/validation-runs/{run_id}/findings"
    ).json()
    assert findings[0]["approved"] is None

    finding_id = findings[0]["id"]
    client.post(
        f"/api/v1/packages/{package_id}/validation-runs/{run_id}/findings/{finding_id}/approval",
        json={"approved": False, "reviewer_notes": "Fix this one."},
    )
    updated = client.get(
        f"/api/v1/packages/{package_id}/validation-runs/{run_id}/findings"
    ).json()
    decided = next(item for item in updated if item["id"] == finding_id)
    assert decided["approved"] is False
    assert decided["reviewer_notes"] == "Fix this one."
    other = next(item for item in updated if item["id"] != finding_id)
